...
export default function EditCollectionDlg({
  collectionId,
  collection,
  update,
  open,
  ...
  const getFileSize = (path: string) => {
    fetch(path, {
      ...
      .then((response) => {
        ...
        const contentLengthStr: string | null =
          response.headers.get('Content-Length')
        const contentLength: number = contentLengthStr
          ? parseInt(contentLengthStr)
          : 0

        // Convert content length from bytes to kilobytes
        const fileSizeInKB = Math.round(contentLength / 1024)
  ...
  const updateCollectionHandler = async () => {
    const updatedLicenses = listedLicenses.filter((license) =>
      selectedLicenses.includes(license.listedId),
    )
    update(
      description,
      collectionName,
      ...
      <DialogContent sx={{ position: 'relative' }}>
        <Box>
                    ...
                    >
                      ...
                      >
                        {collectionImage.imageFile
                          ...
                          : getFileName(collectionImage.imagePath)}
                      </Typography>
                      <Typography
                        sx={{
                          whiteSpace: 'nowrap',
          ...
          <Box display={'flex'} flexDirection={'column'} gap={2} p={1.5}>
            <PrimaryButton onClick={updateCollectionHandler}>
              Save collection
            </PrimaryButton>
          </Box>
        </Box>
